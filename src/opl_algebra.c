...
long opl_bounds_check(long n, const double xmin[],
                               const double xmax[])
{
  if (xmin && xmax) {
    for (long i = 0; i < n; ++i) {
...
void opl_bounds_apply(long n, double x[],
...
void opl_bounds_free(long n, int isfree[],
...
void opl_lower_bound_apply(long n, double x[], double xmin)
{
  for (long i = 0; i < n; ++i) {
    if (x[i] < xmin) {
      x[i] = xmin;
...
void opl_lower_bound_free(long n, int isfree[],
                          const double x[], const double g[],
                          double xmin)
{
  const double zero = 0.0;
...
void opl_upper_bound_apply(long n, double x[], double xmax)
{
  for (long i = 0; i < n; ++i) {
    if (x[i] > xmax) {
      x[i] = xmax;
...
void opl_upper_bound_free(long n, int isfree[],
                          const double x[], const double g[],
                          double xmax)
{
  const double zero = 0.0;
...
void opl_interval_apply(long n, double x[], double a, double b)
{
  if (a > b) {
    double c = a;
    a = b;
...
void opl_interval_free(long n, int isfree[],
                       const double x[], const double g[],
                       double a, double b)
{
  const double zero = 0.0;
...
double opl_dnrm2(long n, const double x[])
{
...
void opl_dscal(long n, double a, double x[])
{
  if (a == 0.0) {
...
void opl_dcopy(long n, const double x[], double y[])
{
  memcpy(y, x, n*sizeof(double));
}

void opl_dcopy_free(long n, const double x[], double y[],
                    const int isfree[])
{
  if (isfree != NULL) {
    const double zero = 0.0;
...
void opl_daxpy(long n, double a, const double x[], double y[])
{
  if (a == 1.0) {
    for (long i = 0; i < n; ++i) {
      y[i] += x[i];
...
void opl_daxpy_free(long n, double a, const double x[],
...
double opl_ddot(long n, const double x[], const double y[])
{
  double s = 0.0;
  for (long i = 0; i < n; ++i) {
    s += x[i]*y[i];
...
double opl_ddot_free(long n, const double x[], const double y[],
                     const int isfree[])
{
  double s = 0.0;
  if (isfree != NULL) {
...
int opl_anyof(long n, const double x[])
{
  for (long i = 0; i < n; ++i) {
    if (x[i]) {
      return 1;
...
int opl_noneof(long n, const double x[])
{
  for (long i = 0; i < n; ++i) {
    if (x[i]) {
      return 0;
...
int opl_allof(long n, const double x[])
{
  for (long i = 0; i < n; ++i) {
    if (! x[i]) {
      return 0;
