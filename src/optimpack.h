 ...
 *
 * VMLM-B is  implemented via two functions:  op_vmlmb_first for initialization
 * and  op_vmlmb_next  for further  iterations.   These  functions use  reverse
 ...
 * call with TASK set to "start".  On exit TASK indicates the required action.
 *
 * The arguments are:
 *
 ...
 *       op_csrch).   Recommended  values: SFTOL=0.0001,  SGTOL=0.9,  SXTOL=0.1
 ...
 *
 *   CSAVE is a character workspace array of length OP_VMLMB_CSAVE_NUMBER (same
 *       as OP_MSG_SIZE) which is used  to store additional information on exit
 ...
 *       approximation to the  solution.  On exit with  TASK=OP_TASK_CONV, X is
 ...
 *       X.
 *
 *   ACTIVE is an  optional integer array with length N  provided by the caller
 *       if the  values in  X has  bounds.  If the  parameters have  no bounds,
 *       ACTIVE  should  be   NULL  (unconstrained  minimization).   Otherwise,
 *       elements set to zero in  ACTIVE indicate that the corresponding values
 ...
 *       successful step).  ACTIVE may also be specified when TASK=OP_TASK_CONV
 *       (i.e.    after   convergence  if   caller   wish   to  continue   with
 *       minimization).  If X has (some)  bounds, the caller is responsible for
 *       applying the  bounds to X before  evaluating the function value  F and
 *       the gradient G (i.e. when TASK=OP_TASK_FG), e.g.:
 *
 ...
 *       greater than zero; else ACTIVE[i] is set to zero if H[i] <= 0 (this is
 *       the only case where ACTIVE is  modified).  As for ACTIVE, H needs only
 *       to be specifed the first time op_vmlmb is called and when JOB=2.
 *
 *   TASK is  the value returned  by op_vmlmb_first and op_vmlmb_next.   It can
 *       have one of the following values:
 *
 *           OP_TASK_FG - caller  must evaluate the function and  gradient at X
 *               and call op_vmlm_next.
 *
 *           OP_TASK_NEWX - a new iterate has been computed.  The approximation
 *               X, function F, and gradient G are available for examination.
 *
 *           OP_TASK_CONV -  the search is successful.   The solution, function
