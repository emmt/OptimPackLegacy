...
size_t opl_csrch_get_workspace_size(
    void)
{
    return OPL_CSRCH_WORKSPACE_SIZE;
}

opl_csrch_workspace_t* opl_csrch_create_workspace(
    void)
{
...
void opl_csrch_destroy_workspace(
...
opl_status_t opl_csrch_iterate(
    opl_csrch_workspace_t* ws,
    double f,
    double g,
    double *stp_ptr)
...
#   define brackt  (ws->brackt)
    ...
    if (stage == 1 && f <= fx && f > ftest) {
        ...
        opl_status_t status = opl_cstep(
            (opl_context_t*)ws, &brackt, stmin, stmax,
            &stx, &fxm, &gxm,
            &sty, &fym, &gym,
            &stp,  fm,   gm);
