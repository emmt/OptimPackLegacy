   ...
   SEE ALSO: optim_vmlm, optim_cgmn, minpack1_umobj. */
{
    ...
    for (;;) {
        ...
        if (task >= OPL_TASK_NEWX) {
            ...
            if (! am_subroutine()) {
                grow, result, [[iter, eval, 1e3*cpu, f, gnorm, step]];
            }
            ...
            if (maxiter >= 0 && iter > maxiter) {
                task = OPL_TASK_WARN;
                msg = swrite(
                    format="warning: too many iterations (%d)\n",
                    iter);
                break;
            }
            if (maxeval >= 0 && eval > maxeval) {
                task = OPL_TASK_WARN;
                msg = swrite(
                    format="warning: too many function evaluation (%d)\n",
                    eval);
                break;
            }
        }
...
/* DOCUMENT minpack1_umipt(n, prob, factor)
     ...
     subroutine minpack1_umobj.  The function returns a vector X of N elements,
     X is a multiple (times FACTOR, default 1.0) of the standard starting
     point.  For the seventh function the standard starting point is 0.0, so in
     this case, if FACTOR is not unity, then the function returns X filled with
     FACTOR.  PROB has the same meaning as in minpack1_umobj.
