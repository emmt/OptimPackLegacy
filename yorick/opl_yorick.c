...
static void vmlmb_free(
    void* ptr)
{
    opl_vmlmb_destroy(((vmlmb_object_t*)ptr)->ws);
}

static void vmlmb_print(
    void* ptr)
{
    char buffer[128];
...
static vmlmb_object_t* get_vmlmb(
    int iarg)
{
    ...
    int argc)
{
    ...
    vmlmb_object_t* obj = (vmlmb_object_t*)ypush_obj(
        &vmlmb_type, sizeof(vmlmb_object_t));
    critical();
    ...
    /* Configure VMLMB instance (adding +1 to iarg's because an element has
       been pushed on top of the stack).. */
    ...
    int argc)
{
    ...
    int iarg = argc;
    vmlmb_object_t* obj = get_vmlmb(--iarg);
    double* x = (double*)get_array(--iarg, Y_DOUBLE, obj->dims, "x", FALSE);
    long fref = yget_ref(--iarg);
    if (fref < 0) {
        y_error("expecting a simple variable reference for argument `f`");
    }
    double f = ygets_d(iarg);
    double* g = (double*)get_array(--iarg, Y_DOUBLE, obj->dims, "g", FALSE);
    int* isfree;
    if (argc >= 5) {
    ...
    opl_task_t task = opl_vmlmb_iterate(obj->ws, x, &f, g, isfree, h);
    ypush_double(f);
    yput_global(fref, 0);
    ypush_long(task);
}

void Y_opl_vmlmb_restart(
    int argc)
{
    if (argc != 1) {
    ...
    int argc)
{
    ...
    double* g = (double*)get_array(--iarg, Y_DOUBLE, obj->dims, "g", FALSE);
    double f;
    opl_task_t task = opl_vmlmb_restore(obj->ws, x, &f, g);
    ypush_double(f);
    yput_global(fref, 0);
    ypush_long(task);
}
